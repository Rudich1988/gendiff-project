...
def create_data_file(file, files_directory='tests/fixtures/'):
    ...
    if isinstance(type(file), dict):
        ...
        if file in [False, True]:
            ...
            file_data = 'null'
        elif file == 'None' and flag is False:
            file_data == 'null'
        else:
            file_data = file
        return file_data
...
def make_diff(data1, data2=None):
    if not isinstance(data1, dict):
        ...
        return create_data_file(data1)
    elif type(data1) is dict and data2 is None:
        return create_data_file(data1)
    ...
    for key in all_files_keys:
        if key in data1 and key in data2 and (type(data1[key]) is not dict or type(data2[key]) is not dict):
            ...
            if type(data1[key]) is not type(data2[key]) and (type(data1[key]) is dict or type(data2[key]) is dict):
                final_diff[key] = (data1[key], data2[key], 'diff types values')
            if key in data1 and key in data2 and data1[key] != data2[key] and type(data1[key]) is type(data2[key]) is dict:
